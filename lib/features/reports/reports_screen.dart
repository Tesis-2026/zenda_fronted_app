...
final reportsInsightsServiceProvider = Provider<InsightsApiService>(
  (_) => InsightsApiService(),
);

final _monthSummaryProvider =
    FutureProvider.family<PeriodSummary, ({int year, int month})>(
      (ref, args) => ref
          .read(reportsInsightsServiceProvider)
          .getMonthSummary(year: args.year, month: args.month),
    );

final _weekSummaryProvider =
    FutureProvider.family<PeriodSummary, ({int year, int week})>(
      (ref, args) => ref
          .read(reportsInsightsServiceProvider)
          .getWeekSummary(year: args.year, week: args.week),
    );

final _comparisonProvider =
    FutureProvider.family<List<MonthComparisonEntry>, int>(
      (ref, months) => ref
          .read(reportsInsightsServiceProvider)
          .getComparison(months: months),
    );
...
final _daySummaryProvider = FutureProvider.family<PeriodSummary, String>(
  (ref, date) =>
      ref.read(reportsInsightsServiceProvider).getDaySummary(date: date),
);
...
class _MonthTabState extends ConsumerState<_MonthTab> {
  ...
  Future<void> _exportPdf() async {
    ...
    final range = await showModalBottomSheet<_ReportRange>(
      ...
      builder: (_) =>
          _ExportRangeSheet(initialYear: _year, initialMonth: _month),
    );
    ...
    try {
      ...
      showAppToast(
        context,
        context.l10n.reportsExportPdfError,
  ...
  Widget build(BuildContext context) {
    ...
    final summary = ref.watch(
      _monthSummaryProvider((year: _year, month: _month)),
    );
    return Stack(
      children: [
        Column(
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: _valid
                      ? () => Navigator.of(
                          context,
                        ).pop(_ReportRange(_fromY, _fromM, _toY, _toM))
                      : null,
...
class _WeekTabState extends ConsumerState<_WeekTab> {
  ...
  void _prev() => setState(() {
    ...
    if (_week < 1) {
      _year--;
      _week = _isoWeekNumber(DateTime(_year, 12, 28));
    }
  });
        ...
        children: [
          ...
          GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final maxSpending =
                  spendingByDay == null || spendingByDay!.isEmpty
                  ? 0.0
        ...
        children: [
          ...
          if (summary.topCategories.isNotEmpty) ...[
            _InsightBullet(
              ...
              text:
                  'El gasto en ${summary.topCategories.first.name} está por encima de tu promedio.',
            ),
            ...
            const _InsightBullet(
              ...
              text:
                  'Ahorraste S/ 760 este mes — 38% de tus ingresos. ¡Supera el 20% recomendado!',
            ),
            const SizedBox(height: 8),
            const _InsightBullet(
              icon: Icons.circle,
              ...
              text:
                  'El gasto en Comida superó el presupuesto por S/ 20 (120% usado). Considera planificar tus comidas.',
            ),
            const SizedBox(height: 8),
            const _InsightBullet(
              icon: Icons.circle,
              ...
              text:
                  'Vas en camino a alcanzar tu meta de Fondo de Emergencia en ~14 meses al ritmo actual.',
            ),
          ],
          if (summary.topCategories.isNotEmpty) ...[
            _InsightBullet(
          ...
          if (summary.topCategories.length > 1) ...[
            ...
            _InsightBullet(
              ...
              text:
                  '${summary.topCategories[1].name} ${l10n.reportsAiInsightsExceeded}',
            ),
          ],
        ],
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: categories.asMap().entries.map((entry) {
        ...
        final displayName = CategoryUtils.labelEs(item.name);
...
class _EvolutionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasAnyData =
        summary.expensesChangePercent != null ||
...
class _CategoriesTabState extends ConsumerState<_CategoriesTab> {
  ...
  ({DateTime from, DateTime to}) get _dateRange {
    ...
    return switch (_period) {
      'week' => (from: now.subtract(Duration(days: now.weekday - 1)), to: now),
      'quarter' => (
  ...
  Widget build(BuildContext context) {
    ...
    if (_period == 'week') {
      final weekSummary = ref.watch(
        _weekSummaryProvider((year: _year, week: _week)),
      );
      ...
      final m2 = ref.watch(
        _monthSummaryProvider((year: prevYear1, month: prevMonth1)),
      );
      final m3 = ref.watch(
        _monthSummaryProvider((year: prevYear2, month: prevMonth2)),
      );
        ...
        Padding(
          ...
          child: SegmentedButton<String>(
            segments: [
              ...
              ButtonSegment(
                value: 'quarter',
                label: Text(l10n.reportsPeriodQuarter),
              ),
            ],
            selected: {_period},
                ...
                : ListView(
                    ...
                    children: [
                      _CategoryBarChart(
                        ...
                        onCategoryTap: (name) => _onCategoryTap(context, name),
                      ),
                    ],
                  ),
            loading: () => const Center(child: CircularProgressIndicator()),
...
String? _categoryNameFromTransaction(Map<String, dynamic> tx) {
  final category = tx['category'];
  if (category is Map<String, dynamic>) {
    return category['name'] as String?;
  }
  return tx['categoryName'] as String?;
}
...
class _CategoryDrillDownSheet extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final displayName = CategoryUtils.labelEs(categoryName);
        ...
        return Column(
          children: [
            ...
            Padding(
              ...
              child: Text(
                l10n.reportsCategoryDrillTitle(displayName),
                style: Theme.of(
                  context,
            ...
            Expanded(
              child: FutureBuilder<List<Map<String, dynamic>>>(
                future: apiService
                    ...
                    .then(
                      (txs) => txs.where((tx) {
                        final txCategoryName = _categoryNameFromTransaction(tx);
                        return txCategoryName?.toLowerCase() ==
                ...
                builder: (context, snapshot) {
                  ...
                  if (txs.isEmpty) {
                    return Center(
                      child: Text(
                        l10n.reportsCategoryNoTransactions,
                        style: TextStyle(
                          color: Theme.of(context).colorScheme.onSurfaceVariant,
                        ),
                      ),
